...
namespace command_sender{   
    void sendtel_ll(const char* serial_str, std::array<char, 16>& base64_payload){
        static const constexpr std::string_view tel_heading = "$SUTEL,";
                
        tel_heading.copy(output_buffer.data(), tel_heading.size());
        
        __builtin_memcpy(output_buffer.data()+tel_heading.size(),
                            base64_payload.data(),
                        base64_payload.size()
        );
        
        output_buffer[tel_heading.size()+base64_payload.size()]='#';
        
        output_buffer[tel_heading.size()+base64_payload.size()+1]   = serial_str [0];
        output_buffer[tel_heading.size()+base64_payload.size()+1+1] = serial_str [1];
        
        output_buffer[tel_heading.size()+base64_payload.size()+2+1] = '*';
        
        uint16_t checksum = checksum::get_checksum(
            output_buffer.begin()+1,
            output_buffer.begin()+tel_heading.size()+2+base64_payload.size()+1
        );
        
        utils::copyashex(checksum, &output_buffer[tel_heading.size()+3+base64_payload.size()+1]);
        
        output_buffer[tel_heading.size()+7+base64_payload.size()+1] = '\r';
        output_buffer[tel_heading.size()+8+base64_payload.size()+1] = '\n';
        
        uart::send_buffer(output_buffer.data(), output_buffer.data()+tel_heading.size()+9+1+base64_payload.size());
    }
